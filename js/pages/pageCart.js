...
const createPageCart = games => {
  ...
  const productsHTML = cartItems.map(gameForCart => `
    ...
    <div class="container-cart-completion">
      <div class="container-cart-value">
        <div class="total-sum-container">
          ...
          <h3 class="value-total" data-value="cart-product-total">R$ ${calculateTotal(cartItems)}</h3>
        </div>
  ...
  const buttonForCart = document.querySelector('[data-button="cart-completion"]');
  //Botao para apagar produto do carrinho 
  const buttonDeleteProductCard = document.querySelectorAll('[data-button="cart-delete"]')
